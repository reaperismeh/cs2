...
public:
	...
	auto initdriver(int ProcID) -> void
	{
	...
	inline bool Patch(uintptr_t address, const unsigned char* bytes, size_t size) {
		// Store original protection
		DWORD oldProtection;
		
		// Change memory protection to allow writing
		if (!VirtualProtectEx(ProcessHandle, (LPVOID)address, size, PAGE_EXECUTE_READWRITE, &oldProtection)) {
			return false;
		}
		
		// Write the new bytes
		SIZE_T bytesWritten;
		bool result = WriteProcessMemory(ProcessHandle, (LPVOID)address, bytes, size, &bytesWritten);
		
		// Restore original protection
		DWORD temp;
		VirtualProtectEx(ProcessHandle, (LPVOID)address, size, oldProtection, &temp);
